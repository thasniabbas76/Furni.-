...
def activate(request, uidb64,token):
    ...
    if user is not None and default_token_generator.check_token(user,token):
        user.is_active = True
        user.save()
        messages.success(request,'Congratulations, Yor account is activated. Now you can loggin here')
        return redirect('login')
    else:
...
def my_orders(request):
    orders = Order.objects.filter(user=request.user,is_ordered = True).order_by('-created_at') #using '-' to get the result in descending order
    context = {
        'orders' : orders,
    }
...
def change_password(request):
    if request.method == 'POST':
        ...
        user = Account.objects.get(username__exact=request.user.username) #check the username as it is in the database
